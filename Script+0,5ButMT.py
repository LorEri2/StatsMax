...
def trouver_plus_longue_serie_plus_0_5_buts_MT(df, nom_equipe):
    """
    Analyse le DataFrame pour trouver la plus longue série de matchs consécutifs
    pour une équipe donnée où le total des buts à la MI-TEMPS était supérieur à 0.5 (1 ou plus).
    """
    print(f"\n--- Début de l'analyse pour : {nom_equipe} (+0.5 Buts MT) ---")
    
    # 1. Vérifier si les colonnes nécessaires existent (HTHG, HTAG)
    ...
    # 4. Calculer le total des buts à la MI-TEMPS et la condition
    # s'assurer que les buts sont numériques, au cas où
    df_equipe['HTHG'] = pd.to_numeric(df_equipe['HTHG'], errors='coerce')
    df_equipe['HTAG'] = pd.to_numeric(df_equipe['HTAG'], errors='coerce')
    
    # Calculer le total Mi-Temps
    df_equipe['TotalHTGoals'] = df_equipe['HTHG'] + df_equipe['HTAG']
    
    # --- LA CONDITION A ÉTÉ MODIFIÉE ICI ---
    # Créer la condition booléenne : Vrai si 1 but ou plus en MT
    df_equipe['Over0_5_HT'] = df_equipe['TotalHTGoals'] > 0 # Équivalent à >= 1

    # 5. Trouver la plus longue série de "Vrai"
    
    # Crée un identifiant unique pour chaque bloc consécutif de Vrai ou Faux
    df_equipe['streak_group'] = (df_equipe['Over0_5_HT'] != df_equipe['Over0_5_HT'].shift()).cumsum()

    # 6. Isoler les séries où la condition est Vraie
    streaks_when_true = df_equipe[df_equipe['Over0_5_HT'] == True]
    ...
    print(f"Nombre total de matchs avec +0.5 buts MT : {len(streaks_when_true)}")
    
    print("\n--- RÉSULTAT ---")
    if max_streak == 0:
        print(f"Aucune série de matchs avec +0.5 buts MT n'a été trouvée pour {nom_equipe}.")
    else:
        print(f"La plus longue série de matchs d'affilée avec +0.5 buts MT est de : {max_streak}")
        ...
        print("Détails des matchs (début et fin de série) :")
        # N'afficher que le début et la fin si la série est longue
        if len(longest_streak_df) > 6:
            print(longest_streak_df[['Date', 'HomeTeam', 'AwayTeam', 'TotalHTGoals']].head(3))
            print("...")
            print(longest_streak_df[['Date', 'HomeTeam', 'AwayTeam', 'TotalHTGoals']].tail(3))
        else:
            print(longest_streak_df[['Date', 'HomeTeam', 'AwayTeam', 'TotalHTGoals']])
...
if __name__ == "__main__":
    ...
    if donnees_completes is not None:
        ...
        trouver_plus_longue_serie_plus_0_5_buts_MT(donnees_completes, equipe_a_analyser)