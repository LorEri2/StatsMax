...
def trouver_plus_longue_serie_plus_1_5_buts(df, nom_equipe):
    """
    Analyse le DataFrame pour trouver la plus longue série de matchs consécutifs
    pour une équipe donnée où le total des buts était supérieur à 1.5 (2 ou plus).
    """
    ...
    # Créer la condition booléenne : Vrai si 2 buts ou plus, Faux sinon
    df_equipe['Over1_5'] = df_equipe['TotalGoals'] > 1.5

    # 5. Trouver la plus longue série de "Vrai"
    
    # Crée un identifiant unique pour chaque bloc consécutif de Vrai ou Faux
    df_equipe['streak_group'] = (df_equipe['Over1_5'] != df_equipe['Over1_5'].shift()).cumsum()

    # 6. Isoler les séries où la condition est Vraie
    streaks_when_true = df_equipe[df_equipe['Over1_5'] == True]
    ...
    print(f"Nombre total de matchs avec +1.5 buts (2 ou plus): {len(streaks_when_true)}")
...
if __name__ == "__main__":
    ...
    if donnees_completes is not None:
        ...
        trouver_plus_longue_serie_plus_1_5_buts(donnees_completes, equipe_a_analyser)