...
def trouver_plus_longue_serie_plus_0_5_buts(df, nom_equipe):
    """
    Analyse le DataFrame pour trouver la plus longue série de matchs consécutifs
    pour une équipe donnée où le total des buts était supérieur à 0.5 (1 ou plus).
    """
    ...
    # 4. Calculer le total des buts et la condition (+0.5 buts)
    ...
    # Créer la condition booléenne : Vrai si 1 but ou plus, Faux sinon
    df_equipe['Over0_5'] = df_equipe['TotalGoals'] > 0 # Équivalent à >= 1

    # 5. Trouver la plus longue série de "Vrai"
    
    # Crée un identifiant unique pour chaque bloc consécutif de Vrai ou Faux
    df_equipe['streak_group'] = (df_equipe['Over0_5'] != df_equipe['Over0_5'].shift()).cumsum()

    # 6. Isoler les séries où la condition est Vraie
    streaks_when_true = df_equipe[df_equipe['Over0_5'] == True]
    ...
    print(f"Nombre total de matchs avec +0.5 buts (1 ou plus): {len(streaks_when_true)}")
    
    print("\n--- RÉSULTAT ---")
    if max_streak == 0:
        print(f"Aucune série de matchs avec +0.5 buts n'a été trouvée pour {nom_equipe}.")
    else:
...
if __name__ == "__main__":
    ...
    if donnees_completes is not None:
        ...
        trouver_plus_longue_serie_plus_0_5_buts(donnees_completes, equipe_a_analyser)