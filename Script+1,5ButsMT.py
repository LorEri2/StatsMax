...
def charger_donnees(fichiers_csv):
    ...
    for f in fichiers_csv:
        try:
            ...
            # Rendre les avertissements plus spécifiques
            if "PremierLeague2019-2020csv.csv" in f and "PremierLeague2019-2020.csv" not in f:
                print(f"Info : 'PremierLeague2019-2020csv.csv' est peut-être 'PremierLeague2019-2020.csv'?")
...
def trouver_record_serie_plus_1_5_buts_MT(df, nom_equipe):
    """
    Analyse le DataFrame pour trouver la plus longue série de matchs consécutifs
    pour une équipe donnée où le total des buts à la MI-TEMPS était supérieur à 1.5 (2 ou plus).
    ...
    # Créer la condition booléenne : Vrai si 2 buts ou plus en MT
    df_equipe['Over1_5_HT'] = df_equipe['TotalHTGoals'] > 1.5 # Équivalent à >= 2

    # 4. Trouver la plus longue série de "Vrai"
    df_equipe['streak_group'] = (df_equipe['Over1_5_HT'] != df_equipe['Over1_5_HT'].shift()).cumsum()
    streaks_when_true = df_equipe[df_equipe['Over1_5_HT'] == True]
...
if __name__ == "__main__":
    ...
    if donnees_completes is not None:
        ...
        for equipe in equipes_actuelles:
            ...
            record_serie = trouver_record_serie_plus_1_5_buts_MT(donnees_completes, equipe)
            
            # Stocker le résultat
            resultats.append({
                'Équipe': equipe,
                'Record_Serie_Plus_1_5_MT': record_serie
            })

        # Étape 4 : Créer un rapport final, le trier et l'afficher
        df_rapport = pd.DataFrame(resultats)
        df_rapport = df_rapport.sort_values(by='Record_Serie_Plus_1_5_MT', ascending=False)
        
        print("\n--- RÉSULTAT FINAL : Record historique de séries +1.5 Buts MT (2+ à la MT) ---")
