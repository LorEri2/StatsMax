...
def trouver_plus_longue_serie_moins_3_5_buts(df, nom_equipe):
    """
    Analyse le DataFrame pour trouver la plus longue série de matchs consécutifs
    pour une équipe donnée où le total des buts était inférieur à 3.5 (3 ou moins).
    """
    ...
    # 4. Calculer le total des buts et la condition (+2.5 buts)
    ...
    # Créer la condition booléenne : Vrai si 3 buts ou plus, Faux sinon
    df_equipe['Under3_5'] = df_equipe['TotalGoals'] < 3.5 # Équivalent à >= 3

    # 5. Trouver la plus longue série de "Vrai"
    
    # Crée un identifiant unique pour chaque bloc consécutif de Vrai ou Faux
    df_equipe['streak_group'] = (df_equipe['Under3_5'] != df_equipe['Under3_5'].shift()).cumsum()

    # 6. Isoler les séries où la condition est Vraie
    streaks_when_true = df_equipe[df_equipe['Under3_5'] == True]
    ...
    print(f"Nombre total de matchs avec -3.5 buts (3 buts ou moins): {len(streaks_when_true)}")
    
    print("\n--- RÉSULTAT ---")
    if max_streak == 0:
        print(f"Aucune série de matchs avec -3.5 buts n'a été trouvée pour {nom_equipe}.")
    else:
        print(f"La plus longue série de matchs d'affilée avec -3.5 buts est de : {max_streak}")
        ...
        if len(longest_streak_df) > 6:
            print(longest_streak_df[['Date', 'HomeTeam', 'AwayTeam', 'TotalGoals']].head(3))
            print("...")
            print(longest_streak_df[['Date', 'HomeTeam', 'AwayTeam', 'TotalGoals']].tail(3))
        else:
            print(longest_streak_df[['Date', 'HomeTeam', 'AwayTeam', 'TotalGoals']])
...
if __name__ == "__main__":
    ...
    if donnees_completes is not None:
        ...
        trouver_plus_longue_serie_moins_3_5_buts(donnees_completes, equipe_a_analyser)