...
def charger_tous_les_matchs(fichiers_csv):
    ...
    for f in fichiers_csv:
        try:
            ...
            # Gérer les variations de nom de fichier de 2019-2020
            if "PremierLeague2019-2020.csv" in f:
                try:
                    df_temp_alt = pd.read_csv("PremierLeague2019-2020csv.csv", parse_dates=['Date'], dayfirst=True)
                    all_dfs.append(df_temp_alt)
                    print("Info : 'PremierLeague2019-2020.csv' non trouvé, 'PremierLeague2019-2020csv.csv' chargé.")
...
def analyser_series_en_cours_plus_0_5(df, equipes):
    """
    Calcule la série en cours pour +0.5 buts (1 ou plus) pour chaque équipe.
    """
    ...
    # Condition : +0.5 buts (1 but ou plus)
    df['ConditionMet'] = df['TotalGoals'] > 0.5 # Équivalent à >= 1
    ...
    for equipe in equipes:
        ...
        if not df_equipe.empty:
            ...
            for condition_remplie in reversed(conditions_list):
                if condition_remplie:  # C'était Vrai (le match avait +0.5 buts)
                    serie_en_cours += 1
                else:  # C'était Faux (le match était un 0-0)
                    break # La série est rompue, on arrête de compter
        
        resultats.append({
            'Équipe': equipe,
            # --- NOM DE COLONNE MODIFIÉ ---
            'Série_en_cours_plus_0_5': serie_en_cours
        })
    ...
    if resultats:
        ...
        # --- CLÉ DE TRI MODIFIÉE ---
        df_rapport = df_rapport.sort_values(by='Série_en_cours_plus_0_5', ascending=False)
        
        # --- TITRE MODIFIÉ ---
        print("\n--- Séries en cours : Matchs avec +0.5 Buts (1 ou plus) ---")
...
if __name__ == "__main__":
    ...
    # Tous les fichiers d'historique (basé sur votre liste précédente)
    fichiers_historique = [
       ...
       "PremierLeague2015-2016.csv", # Corrigé de 201§
       "PremierLeague2016-2017.csv",
       "PremierLeague2017-2018.csv",
       "PremierLeague2018-2019.csv",
       "PremierLeague2019-2020.csv",    # Ou "PremierLeague2019-2020csv.csv"
    ...
    if donnees_completes is not None:
        ...
        if equipes_a_analyser:
            ...
            # --- APPEL DE FONCTION MODIFIÉ ---
            analyser_series_en_cours_plus_0_5(donnees_completes, equipes_a_analyser)